...
class EnvoieSmS(MDScreen):  
    ...
    def validate_recharge_code(self, instance):
        ...
        if self.handle_recharge(code):
            ...
            snack.open()
            self.redema(instance)  # Affiche le snack
    ...
    def check_sms_status(self):
        """Vérifie si les SMS sont expirés ou s'il n'y a plus de SMS restants."""
        today = datetime.date.today()
        expiration_date = datetime.datetime.strptime(
        self.sms_data.get('expiration_date', self.get_expiration_date()), "%Y-%m-%d"
        ).date()

        if today >= expiration_date or self.sms_data.get("sms_count", 0) <= 0:
            # SMS expirés ou plus de SMS → désactiver et réinitialiser
            ...
            Clock.schedule_once(
                lambda dt: self.show_snackbar_rappel(
                    "⚠️ Votre forfait SMS est épuisé ou expiré ! Rechargez pour continuer.", success=False
                ), 1
            )

            return False  # SMS inactifs après expiration ou épuisement
    ...
    def validate_code_content(self, code):
        """Vérifie la validité du code selon les nouvelles règles."""
        if len(code) != 10:
            print("❌ Erreur : Le code n'a pas exactement 10 caractères.")
            return False

        if "15" not in code:
            print("❌ Erreur : Le code ne contient pas '15'.")
            return False

        # Vérifier les 3 premiers chiffres avant "15"
        prefix_3_digits = code[:3]
        used_prefixes = {used_code[:3] for used_code in self.sms_data.get("used_codes", [])}
        if prefix_3_digits in used_prefixes:
            print(f"❌ Erreur : Le préfixe {prefix_3_digits} a déjà été utilisé.")
            return False

        # Vérifier les deux derniers caractères
        last_char = code[-1]  # Doit être '0' ou '9'
        second_last_char = code[-2]  # Doit être 'O', 'S' ou 'C'

        # Règle de rotation
        previous_code = self.sms_data.get("last_used_code", "")
        if previous_code:
            prev_last_char = previous_code[-1]
            prev_second_last_char = previous_code[-2]

            if (prev_second_last_char == 'O' and prev_last_char == '0'):
                allowed_pairs = [('S', '9'), ('O', 'O')]
            elif (prev_second_last_char == 'S' and prev_last_char == '9'):
                allowed_pairs = [('O', '0'), ('S', 'S')]
            else:
                allowed_pairs = [('O', '0'), ('S', '9'), ('S', '0'), ('O', 'O')]

            if (second_last_char, last_char) not in allowed_pairs:
                print(f"❌ Erreur : Les deux derniers caractères '{second_last_char}{last_char}' ne respectent pas la rotation.")
                return False
    ...
    def handle_recharge(self, code):
        ...
        if not self.validate_code_content(code):
            self.show_snackbar("Code invalide (ne respecte pas les règles) ❌", success=False)
            return False

        # Code valide, recharge les SMS
        self.sms_data.update({
            "sms_count": 100,
            "sms_status": "active",
            "expiration_date": self.get_expiration_date(),
            "payment_code": str(self.calculate_payment_code(100)),  # Calcul du code de paiement
            "last_used_code": code
        })
        
        self.sms_data.setdefault("used_codes", []).append(code)
        self.save_sms_status()  # Sauvegarde après toutes les mises à jour
        ...
        try:
            ...
            if "success" in data.lower():
                if self.sms_data['sms_count'] > 0:
                    ...
                    self.show_snackbar( "Échec de l'envoi du message ❌", success=True)
                ...
                self.decrement_sms_count()
                self.show_snackbar("Message envoyé avec succès ! ✅", success=False)
    ...
    def show_snackbar_rappel(self, message, success=True):
        """Affiche une notification Snackbar pendant 7 secondes."""
        snackbar = MDSnackbar(
            MDLabel(
                text=message,
                theme_text_color="Custom",
            ...
            md_bg_color=(0.2, 0.2, 0.2, 1) if success else (0.2, 0.2, 0.2, 1),  # Vert si succès, rouge sinon
            duration=7  # Affichage pendant 7 secondes
        )
        snackbar.open()
